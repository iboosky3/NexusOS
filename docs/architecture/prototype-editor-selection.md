...
3. **GrapesJS 的边界**：开源内核、Webpage Preset 和多个开源扩展已提交，但新增组件的端到端拖放及交接回归尚未完成。为保护 NexusOS 同源工作台，原始 HTML 导入、用户脚本、外部媒体和依赖脚本的演示组件暂不开放。若这些能力成为必须项，先设计独立源隔离与媒体服务，再扩展资源协议。进度见[开发日志](../development/2026-09-22-grapesjs.md)。
