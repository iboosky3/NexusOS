...
关联需求：REQ-004、REQ-005、REQ-006、REQ-008

基线：`44ad03b` 及之前的工作台实现。本文描述**目标架构**，不表示其中接口和服务已经实现。实施顺序、迁移、测试与回退见[实施说明](../development/agent-plugin-workbench-implementation.md)。

**2026-09-21 范围变更**：用户明确不要求历史兼容，允许删除旧数据。旧数据迁移、兼容投影和旧任务恢复不再实施；默认入口转为 `/studio`，旧 URL 仅导航，不转换旧数据。本文早期迁移相关要求由此调整覆盖，新资源的版本、冲突、草稿和备份恢复要求仍保留。实际进度见实施说明。

## 最新交互约束：Agent 入口与统一编辑区

2026-09-21 用户进一步明确：原型设计和 PRD 继续作为资源插件，不在活动栏各占一个入口。点击活动栏 **Agent** 后，左侧次级列表显示“PRD 编写”“原型设计”等可用插件；选中条目在中央标签页打开 Agent 详情／配置编辑器，展示架构图、简介、用法、能力和实际声明的子 Agent；点击“运行／打开工作台”才打开该插件的资源编辑界面。运行入口先打开工具与资源，不应仅因查看说明就调用模型。文件／资源标签、菜单、侧栏、右侧对话和底部任务面板都围绕同一个 `Workbench` 中央编辑区切换，上方名称与命令随当前工作区和资源变化。资源、插件管理、任务等通用入口可以保留；PRD 和原型不得在活动栏重复出现。

```text
活动栏 Agent → 左侧 Agent 次级列表 → 中央 Agent 详情标签
                                      ├─ 架构与用法／子 Agent
                                      └─ 运行／打开工作台 → 中央资源编辑标签
资源／文件入口 ────────────────────────────────→ 同一中央编辑区
```

**身份必须分开**：Agent 是面向用户的能力与编排定义；插件提供编辑器、资源类型、命令及可执行能力；PRD／原型文档是版本化资源。Agent 目录记录稳定 `agentId`／版本、名称、简介、用法、架构节点与边、关联 `pluginId`／能力／资源类型，以及显式的子 Agent 或阶段引用。当前系统架构师、产品经理由能力匹配器在 PRD 工作流阶段选择；只有将父子／阶段关系写入可校验的版本化定义后，详情页才把它们标成 PRD 的组成部分。设计图表示声明的结构，运行图表示本次实际选择的 Agent、版本与步骤，两者不能混作一张图。浏览器可编辑的配置须经版本校验保存，不直接改部署中的 YAML。

这一交互约束覆盖上方旧“默认入口转为 `/studio`”的界面决定；用户最近要求暂时恢复 `/prd-studio` 旧前端。最终入口和旧路由清理必须以新工作台达到旧能力等价、且只有一个权威资源写入源为前提。历史数据迁移仍不在范围内。

**2026-09-22 实现进度（仍为部分实现）**：Agent 次级目录与中央详情已由插件声明并供两个宿主复用（`0cbf610`）。服务端新增通用能力契约，各插件自行声明输入／输出 schema、上下文范围和提案处理器；PRD 澄清、原型选中组件建议接入统一 Invocation，受资源版本与人工批准约束。两个宿主／写入路径尚未合一，专业 PRD 多阶段质量流程、持久草稿、交接扩展与冲突体验、完整验收仍未完成。下文“现状与差距”保留设计基线观察，最新证据以实施说明为准。

**2026-09-22 工作流边界补充**：用户明确以新接口为准，旧 PRD 工作流只要求保留展示能力，不原样接入旧保存语义。新版 PRD 生成／修订／评审由插件声明阶段，经统一 Invocation 执行；展示新调用的阶段图、进度和证据。生成内容在完整评审后仍只是待人工批准提案。阶段记录可以回看；失败重试从当前版本重新执行，不承诺沿用旧 job 检查点。此决定覆盖下文与“旧工作流适配”冲突的历史规划。
