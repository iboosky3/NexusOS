...
打开 `http://127.0.0.1:3000`，点击“编写 PRD”在新标签页打开 `/prd-studio`，也可直接访问该路径。Studio 通过同源 `/api/prd/*` 转发请求，模型密钥不会进入浏览器。旧版 `/prd` 保留供对照使用，共享同一个文档库；新版工具栏提供旧版入口。
...
导入已有 PRD：选择“文件”→“导入 Markdown 文档”，或进入空白 `PRD.md` 页签导入。已有正文时请新建文档导入，或自行在编辑模式合并，避免无提示覆盖。`Ctrl/⌘ S` 保存，`Ctrl/⌘ Shift P` 打开命令搜索。

## 图文编辑与工作台布局

`PRD.md` 支持 Markdown 编辑、预览、分屏、标题、列表、表格和配图。参考材料页签可导入 PNG/JPG/WebP 图片（原文件最多 8 MB），浏览器缩放到最长边 960 像素并转为 JPEG 嵌入正文；透明部分转为白底。图文合计仍受正文 200,000 字符限制，超限会提示，不覆盖已有内容。图片随文档版本和导出文件保存。

流程图页签提供顺序流程编辑，每行一个节点，最多 20 个节点、每个节点 26 字；插入为 `nexus-flow` 代码块，在 Studio 和导出的 HTML 中渲染成 SVG。Markdown 导出保留可编辑代码块，其他 Markdown 阅读器不一定识别该格式。分支图、AI 生成图片与图像理解尚未接入；目前使用手动插图和顺序流程图。
...
左侧能力面板读取真实 Agent / Skill 注册信息，可搜索和展开查看版本、能力与工具需求，运行后标记实际选用项。当前为注册清单查看，不提供插件安装、卸载或任意切换工作流角色；具体选择由后端编排器完成。底部显示真实阶段、输出和待确认项，侧栏、底部面板和 AI 对话均可收起。

## 保存、冲突与恢复

“浏览器草稿”表示只保存在当前浏览器；“已保存”表示服务端保存成功。没有变化的重复保存不会额外创建版本。

新版草稿与最近 40 条澄清对话使用本标签页的 `sessionStorage`，刷新时可恢复；关闭标签页后不能当作持久备份。首次保存会把对话关联到文档 ID。AI 澄清请求携带最近 8 条有长度限制的对话及当前简报，尚未纳入服务端任务追溯。正式生成、修订、评审仍使用持久任务与 SSE 输出。
