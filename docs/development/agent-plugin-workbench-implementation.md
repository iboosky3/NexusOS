...
| M1 第三插件批次（本提交） | 默认停用的示例便签贡献新建命令、编辑器、侧栏和 Agent 能力；只增加插件目录、领域 schema 和装配项，不改 Shell/PRD/原型实现；专项后端测试、生产构建、全仓 Ruff/Mypy 通过；浏览器覆盖便签启用、创建、保存、侧栏、Agent 提案批准 | 这是新 `/studio` 的扩展证明；不代表旧 `StudioWorkspace` 领域分支已迁完，也不代表任意不可信插件可装入 |
