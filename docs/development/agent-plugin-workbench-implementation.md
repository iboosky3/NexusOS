...
本文是[架构方案](../architecture/agent-plugin-workbench.md)的实施计划，目录、API、数据表均为**拟议契约**，不是现有能力清单。本轮只交付文档；后续按阶段修改产品代码，每阶段记录验证证据和剩余边界。
...
- [x] M0 文档作为可审阅基线交付；实现状态继续保持“已规划”。`mkdocs build --strict` 与 `git diff --check` 通过。
