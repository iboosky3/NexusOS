...
本轮入口收敛验证：24 项宿主测试、TypeScript、生产构建通过；Chromium 回归包含三个旧 URL 导航到 `/studio`、首页链接、创建/保存/草稿恢复、Agent 批准、截图交接、双窗口冲突、插件启停及第三插件。确定性模型不代表真实模型质量验收。旧 UI 主链已删除，但服务器旧 PRD 路由、残留未引用组件和共享服务解耦仍待后续收敛。
