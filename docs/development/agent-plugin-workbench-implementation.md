...
| M3 Invocation 批次（本提交，部分实现） | 经 AgentResolver → Skill 路由 → LangGraphRuntime，固定 Agent/Skill/handler 版本及上下文，记录 token 使用；提案批准复查版本/插件；事件游标、幂等、服务端重试关联、中断不重放；5 项专项测试与 Ruff 通过，含忽略取消的模型晚回包 | 旧 PRD job/澄清/组件建议尚未统一适配；组件范围、持久检查点恢复、真实模型质量、策略版本撤权仍待实现/验收；当前仅本地单用户单实例 |
