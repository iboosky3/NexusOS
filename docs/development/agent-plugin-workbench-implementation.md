...
### 2026-09-22 Skill 详情与 Agent 协作图（REQ-011）

常用工作台和资源工作台的 Skill 侧栏均能打开中央详情标签，按需读取已注册 Skill 的说明、能力、输入输出、只读 `skill.yaml` 与可编辑的 `instructions.md` 原文。编辑仅作用于 `instructions.md`，采用摘要比对和原子替换；未知 ID、越界指令路径和过期摘要不能写入。Agent 详情改为主 Agent → 阶段 → 参与角色的连线图，阶段目标与能力直接显示在节点内，不再重复呈现阶段列表。图依据插件静态声明，运行时实际选择仍以任务记录为准。

这不是完整的 Skill 管理器：元数据、版本号和历史记录尚无编辑／发布流程；代码目录只读时原文保存也无法成功。后续运行会读到保存后的指令，但 `skill.yaml` 版本不会自动变化。专项 API／仓库测试、前端类型检查和生产构建通过；尚未完成真实浏览器端到端操作验收。

### 2026-09-22 Skill 详情请求返回 HTML 的修复（REQ-011）

上线前本地检查发现，前端请求 `/api/prd/capabilities/skills/{id}` 时，Next 代理尚未允许该新路径；同一开发服务的 `.next` 又被并行生产构建覆盖，导致 Next 返回缺少模块的 500 HTML。原 `prdApi` 无条件解析 JSON，于是页面显示 `Unexpected token '<'`。已将 Skill 详情路径加入代理允许清单，非 JSON 响应改为说明 HTTP 状态的错误信息；生产构建改用独立 `.next-production` 目录，避免覆盖开发服务产物。重启本地服务后，后端直连、Next 代理的 Skill 详情均返回 `200 application/json`，工作台页面返回 200；代理上的过期摘要和未知 Skill 分别返回 JSON 格式的 409、404。再次运行生产构建期间开发服务仍返回 200。浏览器点击与保存操作仍需端到端验收。

### 2026-09-22 GrapesJS 开源内核能力扩充（REQ-010）

自由原型插件现提供 20 种安全组件与 GrapesJS 原生图层、属性、选择器／状态和样式面板；加入页面新增／重命名／删除、本页说明、桌面／平板／手机画布、撤销重做、预览与组件边界。代码查看只读；可下载当前页 HTML/CSS 和完整工程 JSON。素材库支持本地 PNG/JPEG/WebP 图片，单张上限 45 KB；内置上传也走同一检查，外部 URL 输入关闭。资源仍由插件自有代码装配，宿主无需添加 GrapesJS 专属分支。

确认时逐页截图，并在独立资源中保存同一项目摘要和最多 4 页快照；服务端检查重复页面编号、截图总量与摘要，发布为既有 `nexus.prototype.snapshot` v1。PRD 交接预览会展示所有页面和来源版本。旧单页资源仍可读取。当前 PRD 产物模型限制 4 页、截图合计 140 KB、工程 JSON 200 KB；这些是 NexusOS 交接与存储边界，编辑器 UI 会阻止创建第 5 页。页面切换、历史操作、导出和两页 PRD 交接已在隔离 Chromium 环境中验证。

“全部免费功能”没有可枚举的固定集合：GrapesJS 社区插件持续变化，官方 Studio SDK 是另一产品。当前安全模型仍禁止可执行脚本、外部 URL、视频／地图远程嵌入、HTML 原文导入；尚未提供团队协作、无限媒体库或 AI 组件编辑。旧 `/prd-studio` 宿主适配仍没有统一的草稿与 PRD 交接上下文，原生拖放手势尚缺浏览器自动验收。保留这些缺口，下一阶段按实际设计效率和安全需求选择扩展。

验证：GrapesJS Python 专项 10 项、前端插件宿主 35 项、TypeScript 类型检查、隔离 Next 生产构建、`grapes-prototype-browser.mjs` 与 `grapes-prd-studio-browser.mjs` 均通过；严格文档构建与最终静态检查见本阶段提交记录。
