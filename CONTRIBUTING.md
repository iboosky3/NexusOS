...
1. 代码变更需要覆盖成功、失败和关键边界路径。
2. 修改系统边界、数据所有权、执行语义或技术选型时，同步在 `docs/architecture/architecture-evolution.md` 追加日期、修改内容、修改原因、影响与取舍。
3. 开发、集成、部署或实际运行中确认的问题，同步在 `docs/development/problem-log.md` 记录现象、根因、解决方案、验证方式和遗留风险。
4. 不把静态检查写成编译通过，不把合成基准写成生产效果，不把目标架构写成当前能力。
5. 新增跨语言服务时，同时提供版本化契约、降级路径和对应构建验证。
